...
image: ~/assets/images/cover-pactflow-sso.png
tags:
  - PactFlow
---

import Image from "~/components/common/Image.astro";
import diagramSsoLoginFlow from "~/assets/images/diagram-sso-login-flow.png";
import diagramSsoEntityId from "~/assets/images/diagram-sso-entity-id.png";
import diagramSsoEntityIdProvider from "~/assets/images/diagram-sso-entity-id-provider.png";
import diagramSsoCertificate from "~/assets/images/diagram-sso-certificate.png";
import diagramSsoWebtools from "~/assets/images/diagram-sso-webtools.png";
import diagramSsoWebtoolsCallback from "~/assets/images/diagram-sso-webtools-callback.png";
...
You are probably familiar with this authentication scheme, as there are a lot of websites that allow you to log in to their services using your Google or GitHub account as the identity provider. In fact, these are the two identity providers accepted by PactFlow in all the subscription tiers. To use SAML SSO with a Custom Identity Provider, you should have the [enterprise plan](https://pactflow.io/pricing/).

In this article, we will be talking about On-Premise PactFlow installations, and focusing on the PactFlow side configuration. The identity provider could be [Ping](https://www.pingidentity.com/en.html), [Okta](https://www.okta.com/), [Auth0](https://auth0.com/), or [Azure AD](https://azure.microsoft.com/en-ca/products/active-directory/). It does not matter for the content of this article. But just to satisfy your curiosity, I have been mainly working with [Ping](https://www.pingidentity.com/en.html).
...
If you are familiar with PactFlow, you will know that **basic authentication is highly discouraged**. Actually, PactFlow has recently renamed it to [Demo authentication](https://docs.pactflow.io/docs/on-premises/authentication/demo), so it's hard to be more specific about the use cases for this type of authentication.
...
If you're using the SaaS option, you won't have to struggle with any PactFlow configuration. All the work on your side would be done in the identity provider tool. PactFlow will configure its end to be able to integrate with your IdP platform. The [documentation provided by PactFlow](https://docs.pactflow.io/docs/user-interface/settings/authentication#saml-support) is pretty detailed, and you will be working directly with them to have the SaaS instance running.
...
- **Authorization from an external IdP:** PactFlow will be able to assign roles and teams to the users based on the authorization/entitlements existing on your IdP/AD. Any update on the IdP will be reflected in PactFlow, including the deactivation of access. This feature requires the [SCIM API configuration](https://docs.pactflow.io/docs/on-premises/scim/).
- **Automated user provisioning:** The users can be automatically provisioned in PactFlow when created in your IdP service. Again, this feature requires the [SCIM API configuration](https://docs.pactflow.io/docs/on-premises/scim/).
...
- **Automated user deletion:** Even though the users will lose access to PactFlow if the defined entitlement is revoked, the user will still exist in the platform. This feature requires the [SCIM API configuration](https://docs.pactflow.io/docs/on-premises/scim/).

### SCIM API & Authorization

As you can see, some of the most interesting features depend on the **SCIM API**. This is a [recent feature introduced by PactFlow in December 2022](https://pactflow.io/blog/pactflow-now-supports-scim-api/). I'm beginning the process of incorporating this API with one of our largest contract testing clients. I'll create a new entry in this blog to give our opinion and some advice on the configuration once we have completed the provisioning of the SCIM API.
...
<Image
  src={diagramSsoLoginFlow}
  widths={[400, 1024]}
  sizes="(max-width: 1024px) 400px, 1024px"
  alt="SSO login flow diagram"
  class="mx-auto"
/>
...
This phase will imply the deployment of PactFlow with the minimum SSO SAML configuration included, just to have the instance up & running and the metadata URL available. To achieve this, the main requirement is to have configured [PACTFLOW_SAML_AUTH_ENABLED](https://docs.pactflow.io/docs/on-premises/environment-variables/#pactflow_saml_auth_enabled). The section for your SSO environment variables could look like this when deploying your container:
...
- [PACTFLOW_SAML_ISSUER](https://docs.pactflow.io/docs/on-premises/environment-variables/#pactflow_saml_issuer): This is just an id for your application. It's used to identify your application on the IdP side. It should be unique. From our side, to keep it simple, we tend to use the PactFlow DNS/FQDN.
- [PACTFLOW_SAML_IDP_NAME](https://docs.pactflow.io/docs/on-premises/environment-variables#pactflow_saml_idp_name): This is the text that will be used as a link to the IdP SSO URL on the login page. We tend to use just _"SSO Login"_ text because many developers will not even know they are relying on Azure AD or Ping for the login process.

- [PACTFLOW_SAML_IDP_METADATA_URL](https://docs.pactflow.io/docs/on-premises/environment-variables/#pactflow_saml_idp_metadata_url): This should theoretically point to the metadata URL published by the IdP provider. That will be shared by the IdP once PactFlow is configured as SP. In our case, we don't have this information yet so we should include a fake URL.
- [PACTFLOW_SAML_XXX_ATTRIBUTE](https://docs.pactflow.io/docs/on-premises/environment-variables/#pactflow_saml_idp_id_attribute): List of attributes required by PactFlow to map the user information when creating users in its own database. The values of these fields should be agreed upon with the IdP provider, they are responsible for specifying the attribute value where you should gather that information. In the example, we put some typical values from Azure AD. You may have this information at this point, or not, just keep in mind that you should update them if you didn't set the correct values at this moment.
...
<Image
  src={diagramSsoEntityId}
  widths={[400, 1024]}
  sizes="(max-width: 1024px) 400px, 1024px"
  alt="SSO entity ID in metadata"
  class="mx-auto"
/>
...
As you can see, the PartnerSpId corresponds to the value we configured in our deployment as PACTFLOW_SAML_ISSUER. This example SSO URL is based on [Ping](https://www.pingidentity.com/en.html) approach.
...
You're lucky, this is the simplest path. You will just need to add that URL as [PACTFLOW_SAML_IDP_METADATA_URL](https://docs.pactflow.io/docs/on-premises/environment-variables/#pactflow_saml_idp_metadata_url) and your final configuration will be pretty similar to the dummy deployment one we shared before. Let's suppose the IdP metadata URL is: `https://idpservice.com/idp/pactflow/metadata`
...
This is the most common scenario. In this case, we cannot use [PACTFLOW_SAML_IDP_METADATA_URL](https://docs.pactflow.io/docs/on-premises/environment-variables/#pactflow_saml_idp_metadata_url), so our option will be to use the following three variables:

- [PACTFLOW_SAML_IDP_SSO_TARGET_URL](https://docs.pactflow.io/docs/on-premises/environment-variables/#pactflow_saml_idp_sso_target_url)
...
- [PACTFLOW_SAML_IDP_ENTITY_ID](https://docs.pactflow.io/docs/on-premises/environment-variables/#pactflow_saml_idp_entity_id)

Entity ID from the IdP provider, we can have that value from the entityId attribute in the IdP metadata.

<Image
  src={diagramSsoEntityIdProvider}
  widths={[400, 1024]}
  sizes="(max-width: 1024px) 400px, 1024px"
  alt="SSO entity ID in provider metadata"
  class="mx-auto"
/>

- [PACTFLOW_SAML_IDP_CERT_FINGERPRINT](https://docs.pactflow.io/docs/on-premises/environment-variables/#pactflow_saml_idp_cert_fingerprint)
...
<Image
  src={diagramSsoCertificate}
  widths={[400, 1024]}
  sizes="(max-width: 1024px) 400px, 1024px"
  alt="SSO certificate in metadata"
  class="mx-auto"
/>
...
<Image
  src={diagramSsoWebtools}
  widths={[400, 1024]}
  sizes="(max-width: 1024px) 400px, 1024px"
  alt="SSO login request in browser dev tools"
  class="mx-auto"
/>
...
<Image
  src={diagramSsoWebtoolsCallback}
  widths={[400, 1024]}
  sizes="(max-width: 1024px) 400px, 1024px"
  alt="SSO callback in browser dev tools"
  class="mx-auto"
/>
