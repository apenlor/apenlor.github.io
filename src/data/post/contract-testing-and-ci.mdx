...
image: ~/assets/images/cover-contract-testing-pipelines.png
tags:
  - pact
  - CI/CD
---
...
import diagramWebhookAutoCreation from "~/assets/images/diagram-webhook-auto-creation.png";
import diagramWebhookCallback from "~/assets/images/diagram-webhook-callback.png";
import diagramCanIDeployIndependent from "~/assets/images/diagram-can-i-deploy-independent.png";
import diagramUnwantedSteps from "~/assets/images/diagram-unwanted-steps.png";
import diagramVerificationPipeline from "~/assets/images/diagram-verification-pipeline.png";
import diagramPacticipantId from "~/assets/images/diagram-pacticipant-id.png";
import diagramContractNaming from "~/assets/images/diagram-contract-naming.png";
import diagramCanIDeployMultienvironment from "~/assets/images/diagram-can-i-deploy-multienvironment.png";

## Intro

Here we are again, with the third release of the _Road to PactFlow Enterprise_ series.

This time we will deal with **an advanced topic: CI/CD implementation**. But not just at a high level as in [the first article "Contract Testing Workflows"](/blog/contract-testing-workflows), in this one we will go deeper and **talk about very particular situations you are likely to face, and the options you have to deal with them**.
...
**Webhooks are one of the most powerful components of PactFlow**. They give us the power to integrate contract testing with our team workflows, orchestrate our pipelines and automate anything we want in response to a contract-related event. You can read more about webhooks [in this article by Matt Fellows](https://pactflow.io/blog/webhooks/) or in [the official PactFlow documentation](https://docs.pactflow.io/docs/user-interface/settings/webhooks).
...
<Image
  src={diagramWebhookAutoCreation}
  widths={[400, 1024]}
  sizes="(max-width: 1024px) 400px, 1024px"
  alt="Webhook auto-creation flow diagram"
  class="mx-auto"
/>
...
If you're using the Pact CLI instead of direct API calls, the process is much easier as you have the [create-or-update-webhook](https://github.com/pact-foundation/pact_broker-client#create-or-update-webhook) available, so you will not need to worry about checking anything.
...
But, if your organization has access to tools like Terraform you should be able to automate the provider, webhooks, or team onboarding. Check [this Terraform Provider Pact repository](https://github.com/pactflow/terraform-provider-pact).

## Wait for it… can-i-deploy

### Context

If you still don't know what can-i-deploy is, you can read [this Pactflow official post](https://pactflow.io/blog/can-i-deploy/) that summarizes pretty well what is the purpose of this core contract testing operation.

In introductory contents, we used to see the typical diagrams where the consumer pipeline just waits for the PactFlow webhook callback to continue with the can-i-deploy (waiting for the provider verification to happen). You can see an example of what we're talking about in the [_First Execution \- Consumer Driven_ section of the Contract Testing Workflows article](/blog/contract-testing-workflows#first-execution---consumer-driven).

<Image
  src={diagramWebhookCallback}
  widths={[400, 1024]}
  sizes="(max-width: 1024px) 400px, 1024px"
  alt="Webhook callback flow diagram"
  class="mx-auto"
/>
...
<Image
  src={diagramCanIDeployIndependent}
  widths={[400, 1024]}
  sizes="(max-width: 1024px) 400px, 1024px"
  alt="Independent can-i-deploy flow diagram"
  class="mx-auto"
/>

Basically, we're repeating the can-i-deploy request a given number of times and increasing the wait in each iteration a bit ([exponential backoff strategy](https://en.wikipedia.org/wiki/Exponential_backoff)). **This call will be repeated only if the response from PactFlow is that no verification has been published**. Once we have a verification failed or verification succeed, we will continue with the execution. Of course, it will have a limit of retries to avoid getting stuck.
...
The [Pact CLI offers with the can-i-deploy](https://github.com/pact-foundation/pact_broker-client#can-i-deploy) the possibility of using two parameters to configure this behavior:
...
<Image
  src={diagramUnwantedSteps}
  widths={[400, 1024]}
  sizes="(max-width: 1024px) 400px, 1024px"
  alt="Pipeline with unwanted steps diagram"
  class="mx-auto"
/>
...
<Image
  src={diagramVerificationPipeline}
  widths={[400, 1024]}
  sizes="(max-width: 1024px) 400px, 1024px"
  alt="Dedicated verification pipeline diagram"
  class="mx-auto"
/>

You may have an independent step for the verification publish, but keep in mind that from the provider side if you want to validate the contract using the code it must be done by using the Pact dedicated plugin. If you want to use the [Pact CLI](https://github.com/pact-foundation/pact-ruby-standalone/releases) to publish a verification, you would need to have an up & running provider version of the code. I personally find the plugin & code validation approach much more convenient.
...
As a quick definition, the **consumer version selectors** are a bunch of parameters, available in multiple languages and plugins, that allows **you to filter the consumer contracts that are going to be downloaded & validated by a provider.** This is a topic not deeply covered in technical articles, and it can become a headache if not understood and set up properly in your pipelines. You can find a lot of information related to the [PacFlow official documentation](https://docs.pact.io/pact_broker/advanced_topics/consumer_version_selectors), even related directly to your favorite plugins ([like the Junit5 for example](https://docs.pact.io/implementation_guides/jvm/provider/junit5#selecting-the-pacts-to-verify-with-consumer-version-selectors-4312)).

Just to make it a little more complex, imagine that we are working in an enterprise environment that has a dedicated pipeline for deployments. Meaning that the build and creation of a release candidate is done in a different pipeline than the deployment itself. It's a pretty common situation in large companies, although in the demos we handle everything in just one pipeline thread. Being able to generate a repeatable version number is pretty important to make our lives easier, tools [like this NPM package](https://www.npmjs.com/package/absolute-version?activeTab=readme) can be super helpful.
...
As you may know, [a pacticipant is a party that participates in a contract](https://docs.pact.io/pact_broker/advanced_topics/pacticipant) (ie. a consumer or a provider). With that in mind, you may also know that the pacticipant id is defined in code while developing the tests themselves. For example:

<Image
  src={diagramPacticipantId}
  widths={[400, 1024]}
  sizes="(max-width: 1024px) 400px, 1024px"
  alt="Pacticipant ID definition in code"
  class="mx-auto"
/>
...
<Image
  src={diagramContractNaming}
  widths={[400, 1024]}
  sizes="(max-width: 1024px) 400px, 1024px"
  alt="Contract file naming convention"
  class="mx-auto"
/>
